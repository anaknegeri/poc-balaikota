...
const ViewOption = ({ id, value }: { id: string; value: string }) => {
  return (
    <label className='text-foreground has-data-[state=unchecked]:text-muted-foreground relative z-10 inline-flex h-full min-w-8 cursor-pointer items-center justify-center px-2 whitespace-nowrap uppercase transition-colors select-none'>
      {value}
    ...
    <Card className='gap-4'>
      <CardHeader>
        <div className='flex flex-wrap items-center justify-between gap-3'>
          ...
          <div className='bg-muted dark:bg-background/50 inline-flex h-8 shrink-0 rounded-full p-1'>
            <RadioGroup
              value={selectedValue}
              onValueChange={setSelectedValue}
              className='group after:bg-background dark:after:bg-card/64 has-focus-visible:after:border-ring has-focus-visible:after:ring-ring/50 relative inline-grid grid-cols-[repeat(5,1fr)] items-center gap-0 text-xs font-medium after:absolute after:inset-y-0 after:w-1/5 after:rounded-full after:shadow-xs after:transition-[translate,box-shadow] after:duration-300 after:[transition-timing-function:cubic-bezier(0.16,1,0.3,1)] has-focus-visible:after:ring-[3px] dark:after:inset-shadow-[0_1px_rgb(255_255_255/0.15)] [&:after]:translate-x-[calc(var(--selected-index)*100%)]'
      ...
      <CardContent>
        <ChartContainer
          config={chartConfig}
          className="[&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-border dark:[&_.recharts-cartesian-grid_line[stroke='#ccc']]:stroke-card [&_.recharts-cartesian-axis-line]:stroke-border dark:[&_.recharts-cartesian-axis-line]:stroke-card aspect-auto h-72 w-full [&_.recharts-rectangle.recharts-tooltip-cursor]:fill-(--chart-1)/10 [&_.recharts-rectangle.recharts-tooltip-inner-cursor]:fill-(--chart-1)/25"
        >
          <LineChart
            accessibilityLayer
            key={selectedValue}
