...
export function StatsCard({ title, value, change, icon }: StatsCardProps) {
  ...
  const trendColor = isPositive ? 'text-indigo-500' : 'text-red-500'
  ...
  return (
    <div className='group before:from-input/30 before:via-input before:to-input/30 relative p-4 before:absolute before:inset-y-8 before:right-0 before:w-px before:bg-gradient-to-b last:before:hidden lg:p-5'>
      <div className='relative flex items-center gap-4'>
        <ArrowUpRight
          className='absolute top-0 right-0 text-indigo-500 opacity-0 transition-opacity group-has-[a:hover]:opacity-100'
          size={20}
          aria-hidden='true'
        />
        {/* Icon */}
        <div className='flex size-10 shrink-0 items-center justify-center rounded-full border border-indigo-600/50 bg-indigo-600/25 text-indigo-500 max-[480px]:hidden'>
          {icon}
        </div>
        {/* Content */}
        <div>
